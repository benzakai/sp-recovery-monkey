...
export default function AbandonedCheckouts() {
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={{ border: '1px solid black', width: '55%', borderRadius: '7px', backgroundColor: 'white', padding: '1% 0 5% 0%' }}>
                <div style={{ margin: '6%', fontSize: '60px', fontWeight: '800', color: 'black' }}>Abandoned Carts</div>
                <div style={{ display: 'flex', flexDirection: 'row', marginLeft: '8%', width: '80%' }}>
                    <div style={{ width: '41%', borderRight: '2px solid black' }}>
                        <div style={{ width: '79%' }}>
                            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '10px', justifyContent: 'start', margin: '10px 0 10px 0' }}>
                            ...
                            <div style={{ fontSize: '20px' }}>Abandoned Carts</div>
                        </div><br />
                        <div style={{ width: '79%' }}>
                            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '10px', justifyContent: 'start', margin: '10px 0 10px 0' }}>
                            ...
                            <div style={{ fontSize: '20px' }}>Unlockable Revenue</div>
                        </div>
                    </div>
                    <div style={{ width: '59%', fontSize: '13px', paddingLeft: '5%' }}>
                        {getData?.map((item) => {
                            return (
                                <div style={{ display: 'flex', flexDirection: 'row', margin: '5px 0' }} key={item.id}>
                                    <div style={{ width: '15%' }}><svg style={{ width: '17px', height: '17px' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304l-91.4 0z" /></svg></div>
                                    <div style={{ width: '49%' }}>{item.updated_at ? item.updated_at.split("T")[0] : 'N/A'}</div>
                                    <div style={{ width: '45%' }}>{item.customer.first_name ? item.customer.first_name : 'N/A'} {item.customer.last_name ? item.customer.last_name : 'N/A'}</div>
                                    <div style={{ width: '43%' }}>${item.total_price ? item.total_price : 'N/A'}</div>
                                    <div style={{ width: '15%' }}><svg style={{ width: '17px', height: '17px' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z" /></svg></div>
                                </div>
                            )
                        })}
