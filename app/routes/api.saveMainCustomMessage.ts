...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    try {
        ...
        const firstMessage = await fireStoreFetchService("MainCustomMessage", session.shop);

        if (!firstMessage) {
            console.error("No records found in MainCustomMessage for this shop");
            ...
            const updateMessage = await fireStoreCreateService("MainCustomMessage", session.shop, {
                shop: session.shop,
                header,
                content
            }, {});
